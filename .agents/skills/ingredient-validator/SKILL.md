...
- Category names use sentence case: capitalize the first word and proper nouns only. Generic class words (`liqueur`, `rum`, `vermouth`), common descriptors (`aged`, `blended`, `black`), and fruit names are lowercase — e.g. `Peach liqueur`, `Blended aged rum (Jamaica)`, `Brandy (apple)`, `Crème de Mûre`. `yarn check-data` auto-fixes category-name casing and cascades the fix to every reference.
